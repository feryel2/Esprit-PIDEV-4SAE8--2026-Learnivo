...
public interface EventRegistrationRepository extends JpaRepository<EventRegistration, Long> {
    ...
    @Query("SELECT er FROM EventRegistration er JOIN FETCH er.event JOIN FETCH er.student WHERE er.event.id = :eventId AND er.student.id = :studentId")
    List<EventRegistration> findAllByEventIdAndStudentIdFetched(@Param("eventId") Long eventId, @Param("studentId") Long studentId);
}
