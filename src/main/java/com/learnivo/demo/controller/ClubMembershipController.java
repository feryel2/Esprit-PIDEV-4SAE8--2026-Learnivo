...
public class ClubMembershipController {
    ...
    @GetMapping("/check")
    public ResponseEntity<Boolean> isMember(@RequestParam Long clubId, @RequestParam String email) {
        return ResponseEntity.ok(membershipService.isMember(clubId, email));
    }
}
