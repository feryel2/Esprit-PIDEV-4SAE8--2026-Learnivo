...
interface BackendEventRegistrationResponse {
    id?: number | string;
    /** Chaîne ISO ou tableau Spring pour LocalDateTime */
...
export interface AdminEventRegistrationFormData {
    ...
    eventId: number | null;
    studentId: number | null;
    /** Date d’inscription (réponse API ou ISO). Spring peut renvoyer un tableau [y, mo, d, h, mi, s]. */
    registeredAt?: string | number[] | null;
}

export interface LiveCheckInPendingDto {
    id: number;
    eventId: number;
    eventTitle: string;
    message: string;
...
export interface LiveCheckInSummaryDto {
    id: number;
    message: string;
    createdAt: string;
...
export interface LiveCheckInParticipantDto {
    studentId: number;
    studentName: string;
    present: boolean;
...
export interface EnglishBingoClassDto {
    id: number;
    label: string;
    imageUrl: string;
}

export interface EnglishBingoWordAdminDto {
    id: number;
    word: string;
    correctClassIds: number[];
}

export interface EnglishBingoAdminOverview {
    classes: EnglishBingoClassDto[];
    words: EnglishBingoWordAdminDto[];
}

export interface EnglishBingoPlayRoundDto {
    gridClasses: EnglishBingoClassDto[];
    wordId: number;
    word: string;
}

export interface EnglishBingoWordItemDto {
    wordId: number;
    word: string;
}

export interface EnglishBingoGameSessionDto {
    gridClasses: EnglishBingoClassDto[];
    words: EnglishBingoWordItemDto[];
}

export interface CrosswordClueDto {
    number: number;
...
export interface EnglishBingoCheckResult {
...
export class DataService {
    ...
    private mapBackendEventRegistration(r: BackendEventRegistrationResponse): EventRegistration {
        const ra = r.registeredAt;
        const registeredAtStr =
            ra != null && (Array.isArray(ra) || (typeof ra === 'string' && ra.length > 0))
                ? this.normalizeLocalDateTimeForApi(ra)
        ...
        return {
            ...
            eventStartTime: r.eventStartTime as string | undefined,
        };
    }
    ...
    private eventRegistrationApiBody(
        payload: AdminEventRegistrationFormData,
        ...
        const raw = payload.registeredAt;
        const hasRaw =
            raw != null &&
            (Array.isArray(raw) || (typeof raw === 'string' && raw.trim() !== ''));
        const registeredAt = hasRaw
            ? this.normalizeLocalDateTimeForApi(raw)
            : new Date().toISOString().slice(0, 19);
    ...
    /**
     * Spring peut renvoyer LocalDateTime en JSON tableau [y, mo, d, h, mi, s] ou en chaîne ISO.
     * Le backend attend une chaîne `yyyy-MM-ddTHH:mm:ss` pour désérialiser en LocalDateTime.
     */
    private normalizeLocalDateTimeForApi(value: string | number[]): string {
    ...
    private formatEventRegistrationHttpError(err: {
        error?: string | { message?: string; error?: string; errors?: Record<string, string> };
    }): string {
        const e = err?.error;
        if (typeof e === 'string') {
            return e;
        ...
        if (e && typeof e === 'object' && e.errors && typeof e.errors === 'object') {
            const parts = Object.entries(e.errors).map(([k, v]) => `${k}: ${v}`);
            if (parts.length) {
                return parts.join(' · ');
            }
        }
        if (e && typeof e === 'object') {
            return (e.error as string) ?? (e.message as string) ?? 'Erreur serveur.';
        }
    ...
    createEventRegistration(payload: AdminEventRegistrationFormData) {
        const eventId = Number(payload.eventId);
        const studentId = Number(payload.studentId);
        if (!Number.isFinite(eventId) || !Number.isFinite(studentId)) {
            this.notification.error('Sélectionnez un événement et un étudiant valides.', 'Inscription impossible');
            return;
        }
        this.isLoading.set(true);
        const body = this.eventRegistrationApiBody(
            { ...payload, eventId, studentId },
            { adminOverride: true }
        );
        this.http.post<EventRegistration>(`${this.apiBaseUrl}/event-registrations`, body).subscribe({
            next: () => {
            ...
            error: (err) => {
                ...
                const errorMsg = this.formatEventRegistrationHttpError(err);
    ...
    updateEventRegistration(id: number | string, payload: AdminEventRegistrationFormData) {
        ...
        const studentId = Number(payload.studentId);
        if (!Number.isFinite(eventId) || !Number.isFinite(studentId)) {
            this.notification.error('Sélectionnez un événement et un étudiant valides.', 'Mise à jour impossible');
            return;
        }
        this.isLoading.set(true);
        const body = this.eventRegistrationApiBody(
        ...
        this.http.put<EventRegistration>(`${this.apiBaseUrl}/event-registrations/${id}`, body).subscribe({
            next: () => {
    ...
    /** Admin : lance un appel de présence pour un événement en cours. */
    createLiveEventCheckIn(eventId: number | string, message?: string) {
        const body = { message: message != null && message.trim() !== '' ? message.trim() : null };
        return this.http.post<LiveCheckInSummaryDto>(`${this.apiBaseUrl}/events/${eventId}/live-check-ins`, body);
    }

    listLiveEventCheckIns(eventId: number | string) {
        return this.http.get<LiveCheckInSummaryDto[]>(`${this.apiBaseUrl}/events/${eventId}/live-check-ins`);
    }

    /** Étudiant : check-ins non encore confirmés (affichés en notification). */
    getPendingLiveCheckIns(studentId: number) {
        return this.http.get<LiveCheckInPendingDto[]>(`${this.apiBaseUrl}/live-check-ins/pending`, {
            params: { studentId: String(studentId) },
        });
    }

    ackLiveEventCheckIn(checkInId: number, studentId: number) {
        return this.http.post<void>(`${this.apiBaseUrl}/live-check-ins/${checkInId}/ack`, { studentId });
    }

    getLiveCheckInParticipants(eventId: number | string, checkInId: number) {
        return this.http.get<LiveCheckInParticipantDto[]>(
            `${this.apiBaseUrl}/events/${eventId}/live-check-ins/${checkInId}/participants`
        );
    }
    ...
    englishBingoAdminOverview() {
        return this.http.get<EnglishBingoAdminOverview>(`${this.apiBaseUrl}/english-bingo/admin`);
    }

    englishBingoCreateClass(label: string, file: File) {
        const fd = new FormData();
        fd.append('label', label);
        fd.append('file', file);
        return this.http.post<EnglishBingoClassDto>(`${this.apiBaseUrl}/english-bingo/classes`, fd);
    }

    englishBingoDeleteClass(classId: number) {
        return this.http.delete<void>(`${this.apiBaseUrl}/english-bingo/classes/${classId}`);
    }

    englishBingoAddWord(word: string, correctClassIds: number[]) {
        return this.http.post<EnglishBingoWordAdminDto>(`${this.apiBaseUrl}/english-bingo/words`, {
            word,
            correctClassIds,
        });
    }

    englishBingoDeleteWord(wordId: number) {
        return this.http.delete<void>(`${this.apiBaseUrl}/english-bingo/words/${wordId}`);
    }

    englishBingoGameSession() {
        return this.http.get<EnglishBingoGameSessionDto>(`${this.apiBaseUrl}/english-bingo/play/game-session`);
    }

    englishBingoRandomRound() {
        return this.http.get<EnglishBingoPlayRoundDto>(`${this.apiBaseUrl}/english-bingo/play/random-round`);
    }

    englishBingoCheck(wordId: number, selectedClassIds: number[]) {
        return this.http.post<EnglishBingoCheckResult>(`${this.apiBaseUrl}/english-bingo/play/check`, {
            wordId,
            selectedClassIds,
        });
    }

    getCrosswordPuzzle(): Observable<CrosswordPuzzleDto> {
        return this.http.get<CrosswordPuzzleDto>(`${this.apiBaseUrl}/crossword/latest`);
    }

    getAllCrosswordPuzzles(): Observable<CrosswordPuzzleDto[]> {
        return this.http.get<CrosswordPuzzleDto[]>(`${this.apiBaseUrl}/crossword`);
    }

    saveCrosswordPuzzle(puzzle: Partial<CrosswordPuzzleDto>): Observable<CrosswordPuzzleDto> {
        return this.http.post<CrosswordPuzzleDto>(`${this.apiBaseUrl}/crossword`, puzzle);
    }

    deleteCrosswordPuzzle(id: number): Observable<void> {
        return this.http.delete<void>(`${this.apiBaseUrl}/crossword/${id}`);
    }
}
