...
export interface AdminToastNotification extends AdminNotification {
    visible: boolean;
}
...
interface CourseRecommendationApiItem {
    course: CourseApiResponse;
    reason: string;
    matchScore: number;
}

interface CourseRecommendationApiResponse {
    source: string;
    strategy: string;
    requestedLimit: number;
    recommendations: CourseRecommendationApiItem[];
}
...
export class DataService {
    ...
    toastNotifications = signal<AdminToastNotification[]>([]);
    unreadNotifications = computed(() => this.notifications().filter(item => !item.read));

    trainings = signal<Training[]>([]);
    publishedTrainings = computed(() => this.trainings().filter((training) => training.status === 'Published'));
    ...
    addNotification(notification: Omit<AdminNotification, 'id' | 'createdAt' | 'read'>) {
        ...
        this.toastNotifications.update((list) => [{ ...item, visible: true }, ...list].slice(0, 4));
        setTimeout(() => this.dismissToast(item.id), 4000);
    }
    ...
    dismissToast(id: number) {
        this.toastNotifications.update((list) => list.filter((item) => item.id !== id));
    }
        ...
        const normalized: Quiz = {
            ...
            difficulty: quiz.difficulty === 'Advanced'
                ...
                : quiz.difficulty === 'Intermediate'
                    ? 'Intermediate'
                    : 'Beginner',
            questions: Math.min(10, Math.max(3, Number(quiz.questions ?? 3) || 3)),
            ...
            status: quiz.status === 'Archived'
                ...
                : quiz.status === 'Published'
                    ? 'Published'
                    : 'Draft',
            publishAt: this.normalizePublishAt(quiz.publishAt),
            items: []
        };
    ...
    async getPublishedTrainingBySlug(slug: string) {
        const training = await this.getTrainingBySlug(slug);
    ...
    async getQuizHint(
        ...
        const payload: { questionId: string; hintLevel: number; userAnswer?: number } = {
            questionId,
            hintLevel
        };
        ...
        return firstValueFrom(
            this.http.post<QuizHintApiResponse>(`${this.quizApiUrl}/${quizId}/hint`, payload)
        ) as Promise<QuizHintResult>;
    }
    ...
    async getCourseRecommendations(
        ...
        const response = await firstValueFrom(
            this.http.post<CourseRecommendationApiResponse>(`${this.courseApiUrl}/recommendations/ai`, {
                goal: options?.goal ?? `Recommend the best next courses after ${training.title}.`,
                preferredLevel: options?.preferredLevel ?? training.level,
                preferredCategory: options?.preferredCategory ?? training.category,
                basedOnCourseId: Number(training.id),
        ...
        return {
            ...
            recommendations: response.recommendations.map((item) => ({
                course: this.fromCourseApi(item.course),
                reason: item.reason,
                matchScore: item.matchScore
            }))
        } satisfies CourseRecommendationResult;
    }
    ...
    getQuizQuestions(quiz: Quiz): QuizQuestion[] {
        return quiz.items.map((item, index) => ({
            ...
            difficulty: this.normalizeQuestionDifficulty(item.difficulty, quiz.difficulty),
            weight: this.normalizeQuestionWeight(item.weight, item.difficulty, quiz.difficulty)
        }));
    }
    ...
    private buildDefaultQuizQuestions(quiz: Omit<Quiz, 'items'>): QuizQuestion[] {
        const templates = [
            ...
            },
            {
                ...
                weight: this.normalizeQuestionWeight(undefined, quiz.difficulty, quiz.difficulty)
            },
            {
    ...
    private fromCourseApi(course: CourseApiResponse): Training {
        return {
            ...
            level: course.level,
            difficultyScore: Number.isFinite(Number(course.difficultyScore)) ? Number(course.difficultyScore) : undefined,
            difficultyLabel: this.normalizeCourseDifficulty(course.difficultyLabel),
            image: course.image,
    ...
    private toQuizPayload(quiz: Quiz) {
        ...
        return {
            ...
            publishAt: quiz.status === 'Published' ? this.toQuizPublishAtApi(quiz.publishAt) : null,
            items: items.map((question) => ({
                ...
                difficulty: this.toQuizDifficultyApi(question.difficulty ?? quiz.difficulty),
                weight: this.normalizeQuestionWeight(question.weight, question.difficulty, quiz.difficulty)
            }))
        };
    }
    ...
    private normalizeQuestionDifficulty(
        difficulty: QuizQuestion['difficulty'] | string | undefined,
        fallback: Quiz['difficulty']
    ): QuizQuestion['difficulty'] {
        if (difficulty === 'Advanced' || difficulty === 'Intermediate' || difficulty === 'Beginner') {
            return difficulty;
        }
    ...
    private normalizeQuestionWeight(
        weight: number | undefined,
        difficulty: QuizQuestion['difficulty'] | string | undefined,
        fallback: Quiz['difficulty']
    ) {
        const numericWeight = Number(weight);
        if (Number.isFinite(numericWeight) && numericWeight >= 1 && numericWeight <= 5) {
            return numericWeight;
        }

        switch (this.normalizeQuestionDifficulty(difficulty, fallback)) {
            case 'Advanced':
                return 3;
            case 'Intermediate':
                return 2;
    ...
    private normalizeCourseDifficulty(difficulty: string | undefined): Training['difficultyLabel'] {
        if (difficulty === 'Advanced' || difficulty === 'Intermediate' || difficulty === 'Beginner') {
            return difficulty;
        }
    ...
    private normalizePublishAt(value: string | null | undefined) {
        if (!value) {
            return null;
        }
        ...
        const localDate = new Date(parsed.getTime() - timezoneOffset * 60000);
        return localDate.toISOString().slice(0, 16);
    }

    private toQuizPublishAtApi(value: string | null | undefined) {
        if (!value) {
            return null;
        }
