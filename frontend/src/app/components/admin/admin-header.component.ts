    ...
    <header class="h-16 bg-white border-b border-border flex items-center justify-between px-8 sticky top-0 z-10">
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3 p-1.5 rounded-xl">
          <div class="w-8 h-8 rounded-lg bg-teal-600 flex items-center justify-center text-white font-bold text-sm">
            {{ userInitial }}
          </div>
          <div class="text-left hidden sm:block">
            <p class="text-xs font-bold leading-tight">{{ userEmail }}</p>
...
export class AdminHeaderComponent {
    ...
    get userEmail(): string {
      return this.auth.getCurrentUser()?.email ?? 'Admin';
    }

    get userRole(): string {
      const role: string = this.auth.getCurrentUser()?.role ?? '';
      const map: Record<string, string> = {
        ADMIN: 'Super Admin',
        PROFESSOR: 'Professor',
        STUDENT: 'Student',
        SOCIETY_AGENT: 'Society Agent',
      };
