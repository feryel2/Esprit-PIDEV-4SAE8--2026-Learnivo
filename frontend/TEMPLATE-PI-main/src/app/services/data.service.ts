...
export class DataService {
    ...
    addClass(pc: PlatformClass) {
        this.classes.update((c: PlatformClass[]) => [...c, { ...pc, id: Date.now() }]);
    }

    updateClass(pc: PlatformClass) {
        this.classes.update((c: PlatformClass[]) => c.map((item: PlatformClass) => item.id === pc.id ? pc : item));
    }

    deleteClass(id: number | string) {
        this.classes.update((c: PlatformClass[]) => c.filter((item: PlatformClass) => item.id !== id));
    }
    ...
    registerForCompetition(competitionId: number | string, name: string, email: string): string | null {
        ...
        const max = comp.maxParticipants ?? Number.POSITIVE_INFINITY;
        ...
        this.competitions.update((list: Competition[]) => list.map((c: Competition) => c.id === competitionId ? updated : c));
        return null; // success
    }
    ...
    joinClass(classId: number | string, name: string, email: string): string | null {
        ...
        const max = cls.maxCapacity ?? Number.POSITIVE_INFINITY;
        ...
        const newStatus: PlatformClass['status'] =
            (cls.enrolled ?? []).length + 1 >= (cls.maxCapacity ?? Number.POSITIVE_INFINITY) ? 'full' : cls.status;

        const updated: PlatformClass = { ...cls, enrolled: [...(cls.enrolled ?? []), student], status: newStatus };
        this.classes.update((list: PlatformClass[]) => list.map((c: PlatformClass) => c.id === classId ? updated : c));
        return null; // success
    }
}
